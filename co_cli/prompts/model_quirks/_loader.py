...
**Approach:** Quirk data lives in markdown files under ``prompts/model_quirks/{provider}/``.
...
Create a file at ``co_cli/prompts/model_quirks/{provider}/{model_name}.md``:
...
# Independent copy — this module is importable without any sibling module
...
def normalize_model_name(model_name: str) -> str:
    ...
    Examples:
        >>> normalize_model_name("qwen3.5:35b-a3b-code")
        "qwen3.5"
...
def list_models_with_quirks() -> list[str]:
    ...
    for provider_dir in sorted(_QUIRKS_DIR.iterdir()):
        ...
        if provider_dir.name.startswith("_") or provider_dir.suffix == ".py":
            continue
