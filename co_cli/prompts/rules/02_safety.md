Side-effectful tools require approval by default (for example shell commands, email drafts, Slack sends, and memory writes).
Safe shell commands may be auto-approved when sandbox isolation is active and the command matches the safe-command allowlist.
