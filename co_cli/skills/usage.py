...
Tracks counters and timestamps only for agent-created skills. Bundled skills
(under co_cli/skills/) and URL-installed skills (frontmatter has source-url)
...
def is_agent_created(name: str, deps: CoDeps) -> bool:
    """Return True iff <name>.md exists in user_skills_dir AND has no source-url."""
    skill_path = deps.user_skills_dir / f"{name}.md"
    if not skill_path.exists():
        return False
    return find_skill_source_url(skill_path) is None
