...
Call `memory_create(content=..., artifact_kind=..., title=..., description=...)` for each durable artifact you identify.
...
2. For each durable cross-turn artifact you can justify from the window, call `memory_create(...)` once.
...
5. Do not output explanatory prose. Call `memory_create` for each artifact. When finished, output exactly the word `Done`.
