"""Clock-driven memory housekeeping for the dream daemon.

Memory merge and decay phases. Reviewer (Plan 1) is the sole transcript reader;
housekeeping here operates only on the durable memory item store + recall
metrics. The companion skill-lifecycle plan adds skill merge / decay phases
into ``run_housekeeping`` after the memory phases.
"""
...
async def run_housekeeping(
    ...
    """Run one full housekeeping pass — merge + decay.

    ``cfg.max_pass_seconds`` bounds the merge phase only (LLM-driven, async).
    Decay is synchronous, bounded by ``MAX_DECAY_PER_CYCLE`` filesystem moves,
    ...
    is set to now so the next tick fires on schedule. Companion skill-lifecycle
    plan adds ``merge_skills`` and ``decay_skills`` calls after the memory phases.
    """
