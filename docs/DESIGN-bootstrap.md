...
| `deps.tool_index` | Native + MCP entries set by `create_deps()` (native via `build_tool_registry()`, MCP via `discover_mcp_tools()`) | Full `dict[str, ToolInfo]` map with per-tool `VisibilityPolicy` enum (`ALWAYS`/`DEFERRED`) |
