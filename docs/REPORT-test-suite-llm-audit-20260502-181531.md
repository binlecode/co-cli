...
|---|---|---|---:|---:|---:|---|
| 1 | `test_processor_applies_compaction_when_a` / compaction proactive | `stop` | N/A | 2 | N/A | Response directly addresses the user's intent to summarize the conversation hist |
| 2 | `test_successful_compaction_resets_skip_c` / compaction proactive | `stop` | N/A | 2 | N/A | Response correctly follows instructions and addresses intent. |
| 3 | `test_summarize_messages_from_scratch_ret` / compaction summarization | `stop` | N/A | 2 | 2 | Model correctly summarized conversation and followed formatting constraints. |
| 4 | `test_summarize_messages_iterative_incorp` / compaction summarization | `stop` | N/A | 2 | N/A | Model correctly updated summary with new test action. |
| 5 | `test_llm_call_returns_non_empty_text` / llm call | `stop` | N/A | 2 | N/A | Model correctly replied with the single word 'PONG' as requested. |
| 6 | `test_llm_call_respects_system_instructio` / llm call | `stop` | N/A | 2 | N/A | Response matches system instruction exactly. |
| 7 | `test_llm_call_threads_message_history` / llm call | `stop` | N/A | 2 | N/A | Directly answers user's question correctly. |
| 8 | `test_refusal_no_tool_for_simple_math` / tool calling: no-tool | `stop` | N/A | 2 | N/A | Correct calculation, direct answer. |
| 9 | `test_tool_selection_shell_git_status` / tool calling: shell | `tool_call` | 2 | N/A | N/A | Tool name and arguments match user request exactly. |
| 10 | `test_tool_selection_shell_git_status` / tool calling: shell | `stop` | N/A | 2 | N/A | Response directly addresses user intent with complete summary. |
| 11 | `test_denied_tool_does_not_execute` / tool calling: denied | `tool_call` | 2 | N/A | N/A | Tool name and args correct. |
| 12 | `test_denied_tool_does_not_execute` / tool calling: denied | `stop` | N/A | 2 | N/A | Response correctly explains denial and addresses intent. |
| 13 | `test_auto_approval_skips_prompt_for_reme` / tool calling: approval | `tool_call` | 2 | N/A | N/A | Correct tool and args matching user request. |
| 14 | `test_auto_approval_skips_prompt_for_reme` / tool calling: approval | `stop` | N/A | 2 | N/A | Correct tool and response. |
...
| compaction summarization | 2 | N/A | 2.00 | 2.00 |
