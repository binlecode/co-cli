# DESIGN: TUI Layer — REPL, Completer, and Slash Commands
...
`co_cli/commands/_commands.py` (command registry and handlers), and
`co_cli/display/_core.py` (terminal output surface).
...
Sub-agent turns inherit the mode via `make_subagent_deps()`, which copies
`base.session.reasoning_display` into the sub-agent's `CoSessionState`.
