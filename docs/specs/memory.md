...
- Extractor-only write path via `save_insight`
- Surgical edit tools for main-agent use (`update_memory`, `append_memory`)
- REPL management (`/memory list/count/forget`)
- Session-summary artifact lifecycle

**Non-goals:**
- FTS indexing of memories (by design — grep-only recall)
...
Memory is flat `.md` files with YAML frontmatter stored in `.co-cli/memory/`. They are never FTS-indexed — all recall is file-based grep. Two kinds coexist in the store: `memory` (extracted observations) and `article` (saved external references — storage path lives here, but FTS indexing of articles is handled by the library subsystem).

The extraction pipeline fires after each clean turn as a background asyncio task, scanning a delta window of new messages and calling `save_insight` for each signal. A cursor in `CoSessionState` tracks which messages have been processed; a failed extraction leaves the cursor unchanged so the delta is retried next turn.
...
flowchart TD
    ...
    subgraph Extraction["fire-and-forget extraction"]
        ...
        Extractor["_insights_extractor_agent"]
    ...
    Turn --> Window --> Extractor --> SaveInsight --> Files
    SaveInsight --> Cursor
...
| `artifact_type` | `str \| null` | no | `"session_summary"` — excluded from normal recall and search |
...
**Note on `name`:** `save_insight` writes an optional `name` field to frontmatter (used as the slug source), but `MemoryEntry` does not expose it at read time — it is write-only metadata not surfaced in the in-memory model.

**Frontmatter validation** (`_frontmatter.py`) runs five validators on load: id type and non-empty, created ISO8601 format, kind/origin_url/tags types, type and description constraints, temporal and relationship field types. Unknown `type` and `artifact_type` values produce `logger.warning` but do not raise — files load regardless.

**Enums** (`_frontmatter.py`):
- `ArtifactTypeEnum`: `SESSION_SUMMARY = "session_summary"`
- `MemoryKindEnum`: `MEMORY = "memory"`, `ARTICLE = "article"`
- `MemoryTypeEnum`: `USER`, `FEEDBACK`, `PROJECT`, `REFERENCE`

### 2.2 Read Path

All read operations are file-based. Memory is never indexed in FTS — see [library.md](library.md) for the knowledge index.
...
_recall_for_context(ctx, query, max_results=5, ...) -> ToolReturn   ← internal
  -> load_memories (all), filter_memories, exclude artifact_type="session_summary"
  -> grep_recall → one-hop related slug expansion (up to 5 related entries)
...
search_memories(ctx, query, *, limit=10, tags=None, ...) -> ToolReturn   ← agent tool
  -> load_memories(kind="memory"), exclude session_summary artifacts
  -> filter_memories + grep_recall (no related expansion)
  -> score always 0.0 — no decay scoring on the memory grep path
...
list_memories(ctx, offset=0, limit=20, kind=None) -> ToolReturn   ← agent tool
  ...
  -> paginated; returns full metadata including artifact_type, type, kind
```

**One-hop related expansion** (`_collect_related_memories`): for each match with `.related` slugs, resolves slugs by matching against `path.stem` of a full memory load. Capped at 5 total related entries across all matches; avoids duplicating already-matched IDs.

### 2.3 Write Path (Extraction)

The main agent has no new-file write tools for memory. All extraction is owned by `_insights_extractor_agent` — a separate `Agent[CoDeps, None]` with `save_insight` as its only tool.
...
_run_extraction_async(delta, deps, frontend, *, cursor_start) -> None
  ...
  -> _insights_extractor_agent.run(window, deps=deps, model=..., model_settings=NOREASON_SETTINGS)
  ->   OTel span "co.insight.extraction"
...
save_insight(ctx, content, type_=None, name=None, description=None,
  ...
  -> OTel span "co.insight.save" with attribute "insight.type"
```

**Extractor prompt** (`memory/prompts/memory_extractor.md`) defines four signal types (`user`, `feedback`, `project`, `reference`) each with `when_to_save`, `how_to_use`, and `body_structure` fields. Hard rules: max 3 `save_insight` calls per window; produce no output text; do not save code patterns, git history, debugging solutions, or CLAUDE.md-documented things.
...
update_memory(ctx, slug, old_content, new_content) -> ToolReturn
  ...
  -> write updated body + refresh fm["updated"] timestamp
...
### 2.6 Session-Summary Artifacts

`/new` writes a memory file directly (bypassing the extractor) with `artifact_type="session_summary"`. These files are:
- Excluded from `_recall_for_context()` by predicate on `ArtifactTypeEnum.SESSION_SUMMARY`
- Excluded from `search_memories()` by the same predicate
- Visible in `/memory list` (no special filter in the REPL path)

The summary content is produced by `index_session_summary()` in `summarization.py`, which uses the last 15 messages of the session. Returns `None` on LLM failure; the artifact write is skipped when `None`.

## 3. Config

| Setting | Env Var | Default | Description |
| --- | --- | --- | --- |
| `memory.recall_half_life_days` | `CO_MEMORY_RECALL_HALF_LIFE_DAYS` | `30` | age decay used in confidence scoring for FTS results (not used in grep-only memory recall) |
...
| File | Purpose |
| --- | --- |
| `co_cli/memory/recall.py` | `MemoryEntry` dataclass; `load_memories`, `load_always_on_memories`, `grep_recall`, `filter_memories`, `exclude_session_summaries` |
...
| `co_cli/tools/insights.py` | `save_insight` — extractor-only write tool; UUID-suffix always-new file creation |
| `co_cli/tools/memory.py` | `_recall_for_context` (internal); agent tools: `search_memories`, `list_memories` |
| `co_cli/tools/memory_edit.py` | `update_memory`, `append_memory` — unregistered surgical edit tools; atomic writes via temp-file + `os.replace`; `_find_by_slug` helper |
...
| `co_cli/commands/_commands.py` | `/memory` REPL subcommands: list, count, forget |
...
| `co_cli/context/summarization.py` | `index_session_summary` — used by `/new` to produce session-summary artifact content |
