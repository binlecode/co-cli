...
| Execution & Jobs | `shell_exec`, `task_start`, `task_status`, `task_cancel`, `task_list`, `task_write`, `task_close` | `shell_exec` hybrid approval (and `pty=True` for output fidelity — isatty/ANSI — never interactive drive); the six `task_*` tools are DEFERRED (loaded via `tool_view`). `task_write`/`task_close` drive a running task's stdin (answer prompts / signal EOF); no per-write approval — `task_start` gates the command |
...
| Vision | `image_view` | DEFERRED; per-turn visibility hides it unless the agent model can see (`check_fn` `_vision_available` → `deps.agent_vision_capable`). Reads a local image and attaches the pixels via `ToolReturn.content` for the agent model to read next turn — there is no separate vision model. Read-only — no approval |

**Total: 39 native tools** — 19 ALWAYS · 20 DEFERRED · 12 explicit approval-gated · the 7 Google tools and `image_view` are additionally visibility-gated per turn by a `check_fn` (Google: credential presence; `image_view`: the agent model can see) (`shell_exec` may also prompt dynamically on the command path). The 20 DEFERRED are the 6 `task_*` (`task_start`/`task_status`/`task_cancel`/`task_list`/`task_write`/`task_close`), the 4 `skill_*` writes (`skill_create`/`skill_delete`/`skill_edit`/`skill_patch`), `session_search`/`session_view`, the 7 Google tools, and `image_view`. `task_write`/`task_close` are DEFERRED, so the 19 ALWAYS floor is unchanged.
...
- **Current state:** 19 ALWAYS tools = **17,674 chars** (ceiling `17_700`).
...
- **`task_start` shares the same `work_dir` contract.** The background-task tool takes the same `work_dir` param with the same anchor: `None` = `workspace_dir`, a relative sub-directory is resolved through the write boundary, and an escaping path (e.g. `../..`) is rejected before the task spawns. The `/tasks run` REPL slash command (no `work_dir` param) likewise anchors to `workspace_dir`, so every shell-launch path — foreground, background tool, and slash command — shares one cwd anchor.
...
A foreground `shell_exec` (non-pty) command that is still running after `shell.yield_window_seconds` (default 20s; `0` disables) is promoted to a background task instead of holding the turn open to the hard timeout. This prevents an unbounded command (`mpv <url>`, `tail -f`, a dev server) from making the REPL appear frozen.

- **Same process, no re-spawn.** `ShellBackend.run_command` drains stdout with a bounded incremental read; when the window elapses while the child is still alive it hands the *live* process back as a `YieldedProcess` (live process + the output already consumed). `shell_exec` then calls `adopt_running_process` (`background.py`), which registers that same process as a `BackgroundTaskState`. The command is never killed-and-re-run, so a non-idempotent command does not double-execute.
- **No re-gate.** The command already cleared the DENY/approval gate before it spawned; adoption is a hand-off of an already-authorized process, not a new execution.
- **Output continuity.** The bytes read before yield seed the task log; the adopt monitor reuses the same live stream to continue draining, so there is no lost prefix, dup, or gap (exactly one reader at a time).
- **Result shape.** On yield, `shell_exec` returns a task handle (`task_id` + partial output, `status="running"`) — interchangeable with a `task_start` task: inspect with `task_status`, stop with `task_cancel`.
- **`pty=True` is exempt** — it keeps the plain hard-timeout behavior.

Implementation: `co_cli/tools/shell_backend.py` (`run_command` incremental read, `YieldedProcess`), `co_cli/tools/background.py` (`adopt_running_process`, shared `_drain_to_log` tail), `co_cli/tools/shell/execute.py` (yield→adopt branch), `co_cli/config/shell.py` (`yield_window_seconds`).

### Interactive command-line drive

Two distinct, deliberately-separated surfaces for terminal interactivity — output *fidelity* on the one-shot path, input *drive* on the background path:

- **`shell_exec(pty=True)` — output fidelity only.** Runs the one-shot command under a stdlib `pty.openpty()` pseudo-terminal so the child sees a TTY (`isatty()` true, ANSI colors, line-buffering) instead of the degraded non-TTY form. It stays blocking and one-shot — same timeout, `work_dir`, and policy gate; `kill_process_tree` still fires on timeout (the pty child runs in its own session). There is **no stdin channel** on this path: raw ANSI is preserved (not stripped), but `pty=True` is never interactive drive. `pty=False` (default) drains stdout with a bounded incremental read loop (it is the auto-yield path — see below); `pty=True` is exempt from auto-yield (its master-fd drain has no `proc.stdout` to hand off) and keeps the plain hard-timeout behavior. No new dependency — stdlib `pty`/`os`/`asyncio` only; the master-fd read treats both Linux `b""` and macOS `OSError`/EIO as clean EOF.
- **`task_write` / `task_close` — interactive drive on the background path.** `task_start` now spawns with `stdin=PIPE`, so a running task's input mouth is open. `task_write(task_id, input, newline=True)` writes UTF-8 to that stdin and drains (the trailing newline is the "submit" case); `task_close(task_id)` closes stdin to signal EOF. The model drives the loop manually — `task_start` → `task_status` (read the prompt) → `task_write` (answer) → `task_status` (read the response) → … → `task_close` or `task_cancel`. Output is drained asynchronously by `_monitor`, so a response is not guaranteed visible the instant `task_write` returns; there is no synchronous request-response framing (deferred). Writing to a finished task or a closed pipe surfaces as a clean `tool_error` (`TaskInputError`), never a crash. **Approval posture:** the gate is at `task_start` (the command), not per-write — driving an approved interactive process (e.g. a `python` REPL) via stdin can reach execution the launch approval did not enumerate; accepted because the risk is bounded by what `task_start` already permits.
Interactive stdin is an **agent-only** affordance — there is deliberately no human slash command to write to a task's stdin (peer-aligned: hermes/codex expose stdin-write only as an agent tool / IDE RPC, never as a human REPL command). A human who needs to drive an interactive process runs it in their own terminal; the agent drives processes it spawns via `task_write`/`task_close`.

Implementation: `co_cli/tools/shell_backend.py` (`run_command` pty branch), `co_cli/tools/background.py` (`spawn_task` `stdin=PIPE`, `write_to_task`, `close_task_stdin`, `kill_task` defensive stdin-close), `co_cli/tools/tasks/control.py` (`task_write`/`task_close`).
...
| `co_cli/tools/system/tool_view.py` | `tool_view` (deferred-tool loader — exact-name reveal, fuzzy "did you mean", `check_fn`-aware remediation) |
| `co_cli/tools/vision/view.py` | `image_view` (read a local image — attaches pixels via `ToolReturn.content` for the agent model to read), `_vision_available` (per-turn visibility `check_fn` → `deps.agent_vision_capable`) |
