...
**Total: 37 native tools** — 19 ALWAYS · 18 DEFERRED · 12 explicit approval-gated · the 7 Google tools and `image_view` are additionally visibility-gated per turn by a `check_fn` (Google: credential presence; `image_view`: the agent model can see) (`shell_exec` may also prompt dynamically on the command path). The 18 DEFERRED are the 4 `task_*`, the 4 `skill_*` writes (`skill_create`/`skill_delete`/`skill_edit`/`skill_patch`), `session_search`/`session_view`, the 7 Google tools, and `image_view`.
