...
- **`/write <id> <input>` slash command** — human symmetry with `/background`. The first whitespace-delimited token is the task id; the remainder of the line is passed to stdin verbatim (spaces, quotes, punctuation preserved — never `shlex`-split) with a trailing newline.

Implementation: `co_cli/tools/shell_backend.py` (`run_command` pty branch), `co_cli/tools/background.py` (`spawn_task` `stdin=PIPE`, `write_to_task`, `close_task_stdin`, `kill_task` defensive stdin-close), `co_cli/tools/tasks/control.py` (`task_write`/`task_close`), `co_cli/commands/write.py` (`/write`).
