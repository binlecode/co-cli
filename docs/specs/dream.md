...
**Memory review** — focused on persona, preferences, and references extracted from the transcript. It carries `user_profile_view` / `user_profile_write` so the per-session reviewer also writes `USER.md` one transcript at a time (the cross-session reconciler is profile synthesis, §2.6). Routing is by **scope** (`memory_review.md`): a fact *about the person* that travels across every project (timezone, language, persona, communication style) goes to `USER.md` via `user_profile_write`; a forward-acting operational rule scoped to a domain or artifact ("squash-merge PRs", "pipe pytest to a log") becomes a memory item via `memory_create` (`kind=rule`/`note`/`article`), even when the user phrased it "always". USER.md writes are a wholesale rewrite-and-consolidate under the char budget, never an append.
...
**Reconciliation reasoning model.** `profile_synthesis.md` is the design of *how* the pass reconciles — it treats the **current profile as a prior** and the **transcripts as evidence that refines it**, not a fresh start that replaces it. Four moves:

1. **Direct facts** — keep every durable profile fact unless a transcript contradicts it. A changed attribute *value* (same attribute, new value — "uses pytest" → "moved to a custom runner") *replaces* the old value rather than keeping both; a genuine contradiction (two statements that cannot both be true) is resolved toward the more recent, consistent evidence and the loser is dropped.
2. **Patterns** — a working-style trait is promoted only when **≥2 sessions** show it; a single-session behavior is provisional and left out. Recurrence across the window is what separates a durable pattern from a one-off.
3. **Volatility test** — a fact is kept only if it would *not* plausibly change within six months absent a deliberate announcement (durable identity vs. session-local state, e.g. "backend engineer who prefers terse answers" vs. "debugging an auth bug today").
4. **Wholesale rewrite under budget** — stay under the char budget by *consolidating* overlapping facts into tighter statements, never by blind-truncating a durable fact; if the write is rejected as over budget, consolidate harder and re-write.

The pass writes **only** the profile — it never creates memory items or skills (those are the per-session reviewer's surface, §1.5). The returned `SessionReviewOutput.summary` is a one-line description of what was reconciled (or that the profile was already accurate and left effectively unchanged).
...
| **Welcome banner** | `Dream` row alongside `Model` / `Memory` / `Tools` / `Dir` (see [tui.md](tui.md) §4 Welcome banner). Reflects live runtime state only (the daemon is shared per CO_HOME; `dream.autostart` is config, not shown here), so the row can't disagree with `/dream`: `running · queue N` (green `running`) or `not running · queue N (on disk)` (dim). Built from local PID-file + queue-directory reads — instantaneous, never stalls startup. |
