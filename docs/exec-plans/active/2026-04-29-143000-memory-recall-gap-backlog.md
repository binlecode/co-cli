...
**Created:** 2026-04-29
**Source:** `docs/reference/RESEARCH-memory-recall-design-families.md` §8 (sessions porting gaps) + §11 (cheap-borrow candidates), distilled after the channel architecture (artifacts/sessions/canon) settled.
**Scope:** Co-cli sessions channel and artifacts channel — open gaps vs. peer implementations.

Already resolved (do not re-implement):
- FTS5 query sanitization (`sanitize_fts5_query` in `search_util.py`, wired in both the sessions and artifacts channels — commit `59e45b4`)
...
**Why:** Tool-invocation history (e.g. "when did co last call memory_create?") is not searchable via the sessions channel.
...
**Why:** Current exclusion is path-equality only (skips the active session). When delegation lands, a forked sub-agent session shares parent-session lineage but a different path — it will appear in sessions-channel results even when it should be excluded as part of the current logical session.
...
**Why:** In-progress session turns are not searchable via the sessions channel. A user asking "what did I say earlier about X?" within a long session gets no sessions-channel results for the current session.
...
**Why:** Without source filtering, non-knowledge hits (Obsidian, Drive) can fill the top-N window before knowledge artifacts appear. Relevant if/when the sessions channel is unified into the artifacts chunks pipeline.
**Effort:** ~15 LOC — add `source` predicate to both FTS and vector SQL arms.
**Dependency:** More useful if the sessions channel ever migrates to a Family-B chunks pipeline sharing `co-cli-search.db`.
...
| --- | --- | --- | --- |
| 1 | G1 bounded summarizer concurrency | ~10 LOC | Directly improves sessions-channel reliability on local Ollama; pure win |
| 2 | G2 tool-metadata in FTS | ~15 LOC | Expands sessions-channel recall coverage meaningfully |
| 3 | G6 component score exposure | ~10 LOC | Low-risk diagnostic improvement for artifacts-channel tuning |
| 4 | G3 parent-session lineage | ~20 LOC | Correctness fix; only urgent when delegation is active |
| 5 | G7 pre-rank source filtering | ~15 LOC | Nice-to-have; only load-bearing if sessions/artifacts unify |
