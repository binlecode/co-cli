...
### 6.12 In-turn delegation — agent-as-tool (Phases 2.5 + 3.5, the two additive capabilities)
**Principle:** from an agent's perspective there is **no "subagent" in its capability surface — only tools.** A task agent is reached by the parent emitting an ordinary tool call; the child runs through the same owned driver and returns a distilled result as the tool result. This is the convergent peer shape — hermes `delegate_task`, openclaw `sessions_spawn`, codex `spawn_agent` all engage delegation as a registered tool through the common dispatch protocol (opencode's un-built `task` placeholder likewise). See the peer survey's "Subagent engagement" subsection.

**Why this milestone, and why immediately after Phase 2:** context isolation is a co must-have for long-running small-model loops — a child runs a bounded subtask in *isolated* context and returns only a summary, a context-management lever the lossy compaction chain cannot supply. And the delegation *interface* is decided by the Phase-2 subagent driver (`run_standalone_owned`), so it must be designed now, not retrofitted after approval (§6.5) and recovery (§6.9) build atop the driver. Hence **Phase 2.5**, before Phase 3/4.

**The A-then-B split (child-tool approval posture = Option B, reached in two steps):**
- **Phase 2.5 — read-mostly child.** The delegated child gets only non-mutating, non-approval-required tools (read/search/web/memory/session/todo). No approval-required tool → **fully decoupled from Phase 3's approval work**, ships on its own. Serves the context-isolation use directly (gather + distill).
- **Phase 3.5 — write-capable child + approval propagation.** Widen the child surface to approval-required tools (shell/file-write) and propagate any approval-required call up to the **parent's** inline prompt (built in Phase 3) — the child asks the user, never bypasses the gate. This reaches Option B's full power. Depends on Phase 3; kept separate from it so Phase 3 stays a clean parity refactor (folding capability into a parity phase would muddy its gate).

**The tool:** one registered `@agent_tool` (e.g. `delegate`) whose execution forks deps, drives the child through `run_standalone_owned`, and returns the child's output as the tool result. It runs through co's normal `dispatch_tools` — no special orchestration layer. Owned-path-only (the graph path never gets it); additive, so the cutover parity gate on existing flows is undisturbed. The tool is excluded from the child's own surface (depth cap = 1 for the cutover).

**Interface seams Phase 2 must build `run_standalone_owned` to accommodate, completed for in-turn use at 2.5** (today it is daemon-shaped; seam list refined by the Phase-2.5 plan critique, 2026-06-25):
1. **Return-as-tool-result** — `run_standalone_owned` already returns the validated model (`run_standalone` discards it); in-turn the `delegate` tool returns `result.summary` as the tool-result content. No driver change.
2. **Parent-turn usage rollup — ALREADY CORRECT, assert only.** `record_usage` writes the shared in-memory `usage_accumulator` (`usage.py:58`); the durable ledger line is written once at the turn boundary (`main.py:130`); `fork_deps` shares the accumulator by reference — so child tokens already roll into the parent turn with no double-count. (The first draft over-scoped this as new work.)
3. **Depth guard** — `run_standalone` never depth-checks (daemons are top-level); in-turn must guard on `agent_depth` (set by `fork_deps`) to cap recursion at 1.
4. **Interrupt/cancel propagation** — `run_turn_owned` catches `CancelledError`; a parent interrupt must cancel an in-flight child (let it propagate; partial-usage-loss-on-cancel is acceptable telemetry-only).
5. **Nested dispatch-semaphore decoupling (the load-bearing fix, CD-M-1).** Every owned-path tool body acquires `deps.tool_dispatch_sem` (the `@agent_tool` wrapper, `agent_tool.py:64`), so `delegate` holds a slot for the child's entire synchronous lifetime; `fork_deps` shares the sem by reference, so a child drawing from the **same** pool starves/deadlocks behind the parent's held slots. Fix: the in-turn child gets its **own** `tool_dispatch_sem` (`fork_deps(..., share_dispatch_sem=False)`), and `delegate` is `is_concurrent_safe=False` (runs alone, not in a `gather` batch with sibling tools). Also: pass the **parent turn's** model settings (new `settings` param on `run_standalone_owned`; the daemon path keeps `settings_noreason`), and do **not** set `child_deps.toolset` in the driver (`run_standalone_owned` sets it from `spec.tool_names` itself, `loop.py:413`).

**Discoverability + when-to-delegate (PO-M-1):** `delegate` visibility is **ALWAYS** (orchestrator-only), overriding the episodic-tool DEFERRED default — a flagship capability whose value depends on the model reaching for it should not be hidden behind a `tool_view` reveal. Pair it with a one-line when-to-delegate heuristic (tool description + orchestrator instruction) anchored on an observable cue ("delegate a multi-step read/search/gather subtask whose intermediate results you won't need to retain; small one-shot lookups inline"), to counter small-model under-firing.

**Phase 3 child-aware design (so 3.5 is an extension, not a retrofit):** inline approval (§6.5) is built to accept a propagated request whose origin is a delegated child (parent `frontend` as the prompt surface), even though no child triggers it until 3.5.

**Out of scope:** synchronous delegation only (the tool call blocks until the child returns); nested multi-level delegation beyond the depth cap (= 1, a *cutover* value not a design ceiling); a parallel-children fan-out scheduler; background/async delegation — later capability questions if a need pulls them.
