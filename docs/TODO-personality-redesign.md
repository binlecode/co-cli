...
Fix stale references to the retired two-layer architecture across four docs.

**ROADMAP**

| Location | Current (stale) | Fix |
|----------|----------------|-----|
| Section 3.2 | Two-layer model, `load_personality()` tool, LLM decides when to load personality | Static prompt + per-turn `@agent.system_prompt` layers; personality always present, never tool-gated |
| Section 3.3 | 5 presets, 4 axes, `PRESETS` dict in `_registry.py`, axis summary, style files | 4 roles, 5 traits, file-driven (`souls/` + `traits/` + `behaviors/`), `VALID_PERSONALITIES` from folder listing |
| Section 11 | Agent system prompt = `finch seed + rules + arena injection` | Static prompt + per-turn `## Soul` block (~2,600 chars for finch) + per-turn arena injection; prompt budget ~7,500 chars per agent with personality |
| Section 16.2 | Arena addendum injected at agent creation via `assemble_prompt()` | Arena addendum must use `@agent.system_prompt` (per-turn), same as personality; never modify `assemble_prompt()` |
| Section 18.2 | `get_agent(personality="finch", arena_mode=True, arena_goal=...)` | Personality and arena config live on `CoDeps` fields, not `get_agent()` parameters |
| Section 2, Five pillars | `Shipped — 5 presets, soul seed always-on` | `Shipped — 4 roles (file-driven), per-turn personality injection` |
| Section 7, behavioral metrics | `soul seed + presets` | `file-driven roles with 5 traits` |
| Section 22.5, CrewAI comparison | `soul seed + character system` / `duplicates soul seeds` | `soul + traits + character system` / `duplicates souls/traits` |
| Section 25, DESIGN-16 entry | `Soul-first prompt design: soul seed, 5 rules, personality-rule interaction` | `Prompt & personality system: static/per-turn split, file-driven roles, 5 traits, structural delivery` |
| Section 25, TODO list | `TODO-co-personality-enhancements.md` (deleted) | `TODO-personality-redesign.md` |
| Phase S | `character presets` | `personality roles` |

**DESIGN-01**

| Location | Current (stale) | Fix |
|----------|----------------|-----|
| Line 364 | `Personality preset for system prompt` | `Personality role name (per-turn injection)` |
| Line 388 | `assemble_prompt() — instructions, soul seed, rules, counter-steering` | `assemble_prompt() — static prompt: instructions, rules, counter-steering` |

**TODO-agentic-loop** (active planning doc — stale section misleads future arena work)

| Location | Current (stale) | Fix |
|----------|----------------|-----|
| §11.1–11.4 | Two-layer model, 4 axes, per-axis `/style terse` overrides, `_registry.py` | Structural per-turn model, 5 traits, role immutable within session, file-driven |
| Line 443 | `No soul seed, no personality axes` | `No soul, no personality traits` |
| Line 662 | `Personality depth — via load_personality tool` | Tool is retired; per-turn injection |
| Line 1152 | `soul seed + presets \| _registry.py` | `file-driven roles \| _composer.py + souls/ + traits/ + behaviors/` |
| Line 1305 | `soul seed + presets is sufficient for MVP` | `file-driven roles — DONE (see DESIGN-16)` |

**CLAUDE.md**

- [ ] Update Files section to reference `TODO-personality-redesign.md`
