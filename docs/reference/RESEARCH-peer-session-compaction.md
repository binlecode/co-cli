...
Foreground `summarize_messages()` LLM call. Replaces in-memory history with 2 messages. Writes `{"type":"compact_boundary"}` to JSONL. Increments `compaction_count` in session metadata. Only mechanism that affects the JSONL.
...
|--------|---------|-----------|--------|
| Auto trigger | `contextWindow - output - 13K` (~85% effective) | 50% of token limit (~524K of 1M) | 85% of context budget (token-only trigger) |
| Manual trigger | `/compact` command | None — auto only | `/compact` command |
| Blocking limit | `effectiveContext - 3K` → forced `/compact` | None | None |
| Token limit | Model-dependent (typically 200K) | 1,048,576 (all Gemini models) | `_DEFAULT_TOKEN_BUDGET = 100K` (cloud); `llm_num_ctx` (Ollama) |
...
- **co-cli** (100K default): threshold at 85% (`85K / 100K`), tight — appropriate for the smallest default budget.
...
**Adoption suggestion** (Priority 3 — low): Add JSON-density ratio (`chars // 2` for dict content) to `_estimate_message_tokens()`. Minimal effort, directly matches fork-cc's approach. CJK awareness is higher effort for marginal benefit in current user base.
