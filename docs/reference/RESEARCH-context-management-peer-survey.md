...
Peers re-verified at HEAD 2026-06-09: `hermes-agent 57775e9e1`, `openclaw 98d5c46530`, `opencode 7c1e61af1e` (branch `dev`), `codex 1547785657`. co at `1acae120` (v0.8.327). **Change since last survey:** openclaw has forked the agent loop in-tree — it no longer depends on `@earendil-works/pi-coding-agent`; only `pi-tui 0.78.0` (TUI frontend) remains in `package.json`, and the core loop now lives under `src/agents/embedded-agent-runner/`.
...
- Live tool-output cap `TOOL_RESULT_MAX_CHARS=8000` head-only (`embedded-agent-subscribe.tools.ts:20`), UTF-16-safe truncation; separate context ceiling caps one result at ≤30% of window.
- No internal threshold — invoked by the in-tree runner on context-overflow detection (`isLikelyContextOverflowError`); optional mid-turn precheck (lane-queued).
