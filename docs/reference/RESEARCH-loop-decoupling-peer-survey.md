...
- **Direction (now scheduled — milestone PHASES 2.5 + 3.5):** register a single delegation tool (parent emits it → `dispatch_tools` runs it → it forks deps and drives the child through `run_standalone_owned` → returns the child's output as the tool result), with hermes/openclaw/codex as line-for-line templates. These are the milestone's **two additive capabilities** (the rest of the cutover is behavior-preserving), admitted because (a) context isolation is a co must-have for long-running small-model loops and (b) the delegation *interface* is decided by the Phase-2 subagent driver, so it must be designed before approval/recovery build atop that driver — not deferred. Both are owned-path-only and additive, so the cutover parity gate is undisturbed. **A-then-B split:** **Phase 2.5** ships delegation with a **read-mostly child** (no approval-required tools → decoupled from Phase 3); **Phase 3.5** widens it to **write-capable children with child→parent approval propagation** (Option B — full power, child always asks the user) once Phase 3's inline approval exists. The four interface seams (return-as-tool-result, parent-turn usage rollup, depth guard, interrupt propagation) are in the milestone's Phase 2.5 entry.

#### Child surface, approval, depth, return — the four delegation sub-axes (3-peer code survey, 2026-06-26)
The agent-as-tool *engagement* is convergent (above); the *policies the child runs under* split. Surveyed across the three peers that have delegation (opencode has none yet), plus co as shipped at Phase 3.5:

| Axis | hermes | openclaw | codex | **co (3.5)** |
|---|---|---|---|---|
| **Child tool surface** | write-capable; **inherit parent ∩ − blocklist** (`delegate_tool.py:1066-1082`); blocklist = `delegate_task, clarify, memory, send_message, execute_code, cronjob` (`44-54`) | write-capable; **inherit allowlist − role/depth deny** (`agent-tools.policy.ts:84-146`); always-denied = `gateway, agents_list, session_status, cron, sessions_send` | write-capable; **inherit = parent** (full surface, `multi_agents_common.rs:204-211`) | write-capable; **curated allowlist** (`_CHILD_TOOL_NAMES`, 14 tools) |
| **Approval of a child's gated call** | **auto-deny default / opt-in auto-approve** — child runs in a worker thread with no interactive callback, never reaches the user (`delegate_tool.py:57-112`) | **sandbox + inherited tool policy** — denied tools silently fail, no user prompt (`agent-tools.policy.ts:115-146`, `subagent-spawn.ts:1251-1272`) | **propagate to the human** — child's approval request forwarded up the parent session to the user (direct or Guardian); child inherits parent `approval_policy` (`codex_delegate.rs:267-312`, `multi_agents_common.rs:253-275`) | **propagate to the human** — child's gated call surfaces on the parent `Frontend` via the `runtime.frontend` seam; headless → auto-deny |
| **Nesting / depth** | configurable `max_spawn_depth` (default flat=1) (`delegate_tool.py:484-520`) | configurable, default 1 (`agent-limits.ts:13`) | configurable `agent_max_depth`; V2 unbounded (`spec_plan.rs:347-354`) | **hard 1, no config** (surface-exclusion + `agent_depth` guard) |
| **Return to parent** | summary only (`final_response`) (`delegate_tool.py:1786,1850`) | latest output only (`subagent-announce-output.ts:302-414`) | **full event stream** minus approval mechanics (`codex_delegate.rs:289-401`) | summary only (`DelegationResult.summary`) |

**Reads:**
- **Write-capable child — 4/4 convergent.** co's 2.5 read-only child was the outlier; 3.5 closes it.
- **Approval handling — three distinct peer models; co = codex.** *propagate* (codex), *auto-deny* (hermes, forced by its threadpool isolation — no user channel exists), *sandbox/policy* (openclaw). co adopts codex's **propagate-to-human** model, and the *mechanism* independently converged: codex forwards the request up the parent session's approval channel; co reaches the parent `Frontend` from inside the child. Principled (the trust boundary holds across the seam), not universal.
- **Return shape — co with the 2/3 majority** (summary-only; codex alone streams the full transcript).
- **Surface construction — co is the outlier.** All three peers **inherit parent − blocklist** (default-allow); co uses a **curated allowlist** (default-deny). The allowlist is partly an artifact of co's **deferred-visibility tier** (peers have flat toolsets): it cherry-picks ALWAYS reads + two DEFERRED reads (`session_view`, `image_view`) and omits the rest. Aligning to the peer default-allow model is a candidate refactor; the trade is small-model schema leanness + default-deny safety vs. auto-tracking newly-added tools into the child.
- **Depth — co most conservative.** Peers all default to 1 too, but expose a config ceiling; co hard-codes 1.
