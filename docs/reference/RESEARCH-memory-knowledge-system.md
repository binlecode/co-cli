...
- **Reference repos read:** `openclaw` (primary), `letta` (primary), `mem0` (primary), `codex` (primary), `claude-code` (secondary), `gemini-cli` (secondary), `tinyclaw` (secondary), `nanobot` (secondary)
...
|----------|----------------|-------------|---------|
| Storage architecture | Two-layer flat: markdown files + SQLite FTS5/sqlite-vec. Single tier — memories and articles coexist, distinguished by `kind` frontmatter. | letta: 3-tier (in-context blocks, archival pgvector, message store). mem0: robust local-first SQLite vector store maturity with explicit SQLite migration capabilities. openclaw: SQLite FTS5 + sqlite-vec chunks. codex: file-backed workspaces without DB indices. tinyclaw: structured monorepo using queues and routers, moving away from flat files. | aligned |
| Search strategy | Default FTS5 BM25; optional hybrid (BM25 + sqlite-vec cosine, weighted merge). Reranker: none / fastembed cross-encoder / ollama / gemini. Fallback chain: hybrid → fts5 → grep. | openclaw: FTS5 + sqlite-vec + temporal decay + optional MMR. letta: hybrid text + semantic. mem0: embedding-first + optional reranker. codex: grep/file-read only. | aligned |
| Memory kinds / taxonomy | Two kinds: `memory` (lifecycle-managed user facts) and `article` (saved references, no lifecycle). External namespaces: `obsidian`, `drive`. No always-in-context block tier. | letta: 3-tier — core blocks (always in context), archival passages (retrieval), conversation recall. codex: evergreen MEMORY.md (always loaded) + dated rollout summaries. mem0: explicit mutation semantics combined with mature local SQLite setup; collapses types (semantic/episodic/procedural) entirely — everything is a fact via the same `add()` pipeline. openclaw: flat chunks, no kind distinction. | divergent |
| Memory lifecycle | Full lifecycle: fuzzy dedup (rapidfuzz token_sort_ratio), LLM consolidation (two-phase extract_facts + resolve), retention cut (oldest non-protected deleted at cap), temporal decay rescoring, touch-on-read gravity. Protected entries exempt from cut and dedup deletion. | mem0: LLM-driven ADD/UPDATE/DELETE/NONE on every `add()` call with explicit update/delete semantics. nanobot: token-aware memory consolidation logic managing budgets during efficient background compaction. openclaw: embedded runner compaction, file-sync, and session data limits. codex: Phase 1 per-rollout extraction; Phase 2 periodic consolidation into MEMORY.md. letta: agent-driven only — no auto-lifecycle. | aligned |
| External knowledge lifecycle | `kind: article` has save/retrieve-only lifecycle — URL dedup on save, no decay, no consolidation, no automatic re-sync. Obsidian and Drive sources sync on-demand at search time (triggered by `search_knowledge` and `read_drive_file`), not continuously. No stale eviction for external sources outside of startup `sync_dir`. | openclaw: chokidar file watcher on memory dirs; session transcript delta tracking (bytes+messages thresholds); hash-based change detection; stale row cleanup after every sync; safe atomic reindex (temp-db swap) on model/chunk config change; failover resilience. mem0: no distinction — all content goes through same `add()` pipeline (LLM fact extraction → contradiction resolution → vector+graph write in parallel). letta: archival passages have no auto-lifecycle; cascade-delete when source deleted. codex: batch post-rollout — trace files → LLM summary → periodic consolidation; no real-time sync. | **gap** |
