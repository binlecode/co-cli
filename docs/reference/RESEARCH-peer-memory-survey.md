...
This document replaces the earlier peer-specific memory research notes with one memory-only survey and compares them to the current `co-cli` implementation.

Superseded memory notes:

- `RESEARCH-memory-fork-cc.md`
- `RESEARCH-memory-hermes-agent.md`
- `RESEARCH-memory-letta.md`
- `RESEARCH-openclaw-memory.md`
...
High-signal read: peers split into two camps. `fork-cc`, `hermes`, and `co-cli` keep Markdown as the canonical memory format; `letta` and `openclaw` move the center of gravity into structured stores. `co-cli` is currently the simplest of the Markdown-first systems in storage topology.
...
| `co-cli` | explicit always-on entries plus per-turn recall injected as trailing system context | narrower retrieval model than `hermes` and `openclaw`, but more dynamic than `hermes` built-in memory and lighter than `fork-cc`'s extra relevance pass |

High-signal read: the main design split is not "files vs database"; it is "session-frozen hot memory" vs "fresh per-turn recall". `co-cli` sits on the fresh-recall side and keeps standing context explicit through `always_on`.
...
| `co-cli` | background extractor owns new writes; `save_memory()` always appends new files; surgical edit helpers exist but are not main-agent tools | deliberately narrower and safer than `fork-cc`/`letta`, but also less capable for active memory maintenance |

High-signal read: `co-cli` treats new memory creation as a lifecycle concern, not a general interactive tool. That keeps the default surface small, but it also means active memory maintenance is underpowered compared with the peers that expose direct edit primitives.
...
| `co-cli` | cursor-based extraction retry discipline, but no built-in dedup in `save_memory()`, no background consolidation loop, no TTL/retention loop in the current memory path | materially narrower than every peer except `letta` on automated hygiene |

High-signal read: this is the clearest gap. Every peer except `letta` invests in some form of background memory hygiene. Current `co-cli` memory is disciplined about when extraction runs, but not about what happens after the files accumulate.
...
High-signal read: `co-cli` is unapologetically single-agent and single-store. That keeps the implementation legible, but it also means the survey peers are solving sharing and isolation problems that `co-cli` currently chooses not to have.
...
- Markdown remains a viable canonical representation for agent memory.
...
- Memory extraction is treated as a lifecycle concern, not just a manual command.
...
- no background consolidation or dreaming loop
...
- no project/user/local/team scope model
...
Current `co-cli` already satisfies the first two in a lightweight way. The missing layer, relative to the peer set as a whole, is corpus hygiene after write time: consolidation, contradiction handling, retention, or trust-weighted decay. That is the clearest stable conclusion carried across the superseded peer-specific notes.
