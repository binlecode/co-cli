...
> **Opencode omitted.** This report covers the two richest compaction implementations; opencode's is materially thinner. Opencode does **not** disk-spill tool output — at HEAD `a78605f8e` it *truncates* in place (`truncateToolOutput`, `TOOL_OUTPUT_MAX_CHARS=2000`; `session/message-v2.ts:281`, `session/compaction.ts:37`), and its compaction runs sequentially emitting only `Compaction.Started`/`Ended`. Co's L1 spill (`co_cli/tools/tool_io.py`: `spill_if_oversized` → `tool-results/<sha16>.txt`) is a content-addressed, recoverable persist that **has no opencode equivalent** — it is co-specific, not an opencode adoption. The other opencode patterns (reserved-buffer trigger `COMPACTION_BUFFER=20_000` in `session/overflow.ts:6`, `splitTurn` in `session/compaction.ts:161`, Task-tool delegation in `tool/task.ts`, plugin hooks) are alternative trigger models co has consciously not adopted or JS-ecosystem details that don't map. See `docs/specs/compaction.md` §2.2.
