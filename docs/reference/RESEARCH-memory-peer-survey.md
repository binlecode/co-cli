...
7. `memU` (medium: memory for 24/7 proactive agents, openclaw ecosystem)
8. `ReMe` (medium: memory management kit focusing on refine/remember operations)
...
7. `ReMe` (memory management toolkit focusing on memory refinement)
8. `memU` (proactive background memory layer)
...
| `memU`         | hierarchical "file system" analogy (Categories -> Items -> Resources), backed by DB/in-memory                              | structured hierarchy designed for 24/7 background agents                                                          |
...
| `ReMe`         | dual model: `ReMeLight` uses explicit markdown files and jsonl raw dialog; vector `ReMe` uses DB                           | `ReMeLight` shares `co-cli`'s Markdown instinct but adds raw dialog persistence                                   |
...
High-signal read: peers split into two camps. `fork-cc`, `goose`, `hermes`, `ReMeLight`, and `co-cli` keep Markdown/text as the canonical memory format; `elizaos`, `letta`, `memU`, and `openclaw` move the center of gravity into structured stores. `co-cli` is currently the simplest of the Markdown-first systems in storage topology while retaining rich metadata.
...
| `memU`         | dual-mode retrieval: RAG (instant context via embeddings) vs LLM (anticipatory reasoning, query evolution)                              | focuses on proactive context loading rather than reactive injection                                                                                                |
...
| `ReMe`         | `pre_reasoning_hook` dynamically truncates/compresses context via ReAct; hybrid search (Vector + BM25)                                  | much more aggressive in-context truncation and compression than `co-cli`                                                                                           |
...
High-signal read: the main design split is not "files vs database"; it is "session-frozen hot memory" vs "fresh per-turn recall". `co-cli` sits on the fresh-recall side and keeps standing context explicit through `always_on`. `elizaos` and `ReMe` highlight the value of token-aware context windowing, truncation, and state composition.
...
| `memU`         | continuous real-time ingestion (`memorize`) performs auto-extraction without delay                                                       | fully backgrounded, continuous ingestion unlike `co-cli`'s turn-based extraction                                        |
...
| `ReMe`         | async `summary_memory` uses ReAct + file tools (`read`, `write`, `edit`) to merge memories                                               | exposes direct file IO tools to the internal summarizer agent, similar to `co-cli`'s edit helpers                       |
...
| `memU`         | auto-categorization and continuous pattern detection as part of the ingestion pipeline                                                                                 | continuous online hygiene instead of batch offline hygiene                          |
...
| `ReMe`         | `compact_memory` aggressively compresses older dialog; tool result cache has time-to-live (TTL) auto-cleanup                                                           | strong focus on context window survival and TTL expiry rather than deep synthesis   |
...
High-signal read: this is the clearest gap. Almost every persistent-memory peer invests in some form of background memory hygiene or TTL expiry. Current `co-cli` memory is disciplined about when extraction runs, but not about what happens after the files accumulate.
...
| `memU`         | user-specific or agent-specific scopes via `where` filters                                                                                      | geared towards multi-agent or multi-user backend deployments                                 |
...
| `ReMe`         | Vector DB separated into Personal, Procedural, and Tool memory types                                                                            | taxonomy-based scoping rather than project/user scoping                                      |
...
- no background consolidation or dreaming loop (unlike `elizaos`, `fork-cc`, `openclaw`, `memU`, `ReMe`)
- no memory-provider/plugin architecture (unlike `elizaos`, `hermes`)
- no explicit lifecycle/TTL management for expired memories (unlike `ReMe`)
