...
- **Owned loop is the default and only turn (`co_cli/config/llm.py`, `co_cli/main.py`, `co_cli/agent/run.py`)** — the `use_owned_loop` flag is removed; `_run_foreground_turn` and `run_standalone` call `run_turn_owned` / `run_standalone_owned` unconditionally. The `agent` parameter/field threading and the `Agent[CoDeps, str | DeferredToolRequests]` annotations are deleted from `main.py` and `commands/types.py` (`CommandContext.agent`).
- **Graph orchestration deleted** — `co_cli/agent/orchestrate.py` (~1100 lines: `run_turn`, the `agent.iter()` driver, the approval/deferred suspend-resume loop, `_StallTimer`, the graph-only event handlers, and every recovery/length-retry helper now owned by the Phase-4 twins in `recovery.py`/`loop.py`/`preflight.py`) and `co_cli/agent/build.py` (`build_orchestrator`, `build_task_agent`) are removed.
- **Shared symbols relocated to owned homes** — `TurnResult` + `TurnOutcome` → `co_cli/agent/turn_state.py`; `_handle_model_request_event` → `co_cli/display/stream_renderer.py` as `handle_model_request_event` (underscore dropped — it is now imported across packages, per the visibility contract); `_REASONING_OVERFLOW_MESSAGE` / `TOOL_CAP_NO_ANSWER_TEXT` / `_last_assistant_text` → `co_cli/agent/loop.py`.
- **`SurrogateRecoveryModel` wrapper deleted (`co_cli/llm/factory.py`, `co_cli/llm/surrogate_recovery_model.py`)** — `build_model` now constructs the raw `OpenAIChatModel`/`GoogleModel`; the owned `model_turn` client reimplements span + surrogate-retry + JSON repair inline, so `loop._unwrap_model` is gone (the model is already raw). `_json_repair` / `_message_sanitize` / `RepairingStreamedResponse` survive — the owned `model_turn` uses them.
- **`_CallSeamToolset` wrap-then-unwrap collapsed (`co_cli/agent/core.py`, `co_cli/agent/dispatch.py`, `co_cli/agent/toolset.py`)** — `assemble_routing_toolset` returns the visibility-filtered routing stack directly; `dispatch_tools` dispatches on `deps.toolset` (no `.wrapped` reach-in) and hosts the tool span, per-model-request cap, and MCP spill as straight-line code. The `_CallSeamToolset` class is deleted.
- **Orphan approval/deferred wiring removed** — `resume_tool_names` + its visibility-filter resume gate (`deps.py`, `toolset.py`), `record_approval_choice` (`tools/approvals.py`), the five `*_prompt_ctx` graph shims + `ORCHESTRATOR_SPEC.per_turn_instructions` tuple + the `OrchestratorSpec.per_turn_instructions` field (`_instructions.py`, `orchestrator.py`, `spec.py`). Keepers (`clarify_answers`, `deferred_tool_awareness_prompt`, `QuestionRequired`) survive — the owned path uses them.
- **Tests + evals redirected onto the owned path** — 11 graph-only test files deleted (covered by `test_flow_owned_*` twins), wrapper-pinned behavior tests (JSON repair, tool span/cap/spill) ported to the surviving owners, `evals/_deps.drive_turn` calls `run_turn_owned` unconditionally (the `CO_EVAL_OWNED_LOOP` env is gone). Source docstrings/comments swept of stale graph references repo-wide.
- **Scaffolding tenet confirmed, not abstracted (`co_cli/agent/loop.py` docstring)** — the orchestrator and subagent drivers share the request primitive, every preflight builder, `dispatch_tools`, `collect_inline_approvals`, and `ToolCapState`, differing only by workflow; the one intended construction split (FLAT_EXACT plain `FunctionToolset` vs the VISIBILITY_MODEL filtered routing stack) is recorded as the baseline. No parameterized mega-loop was introduced.
...
- **Typed provider-error classification (`co_cli/agent/recovery.py`, new)** — `classify_provider_error` turns a raised provider exception into a typed `ErrorClass` (action: recover-overflow / reflect-400 / terminal; the `TurnExit` reason; the user-facing message; the turn-span event). The graph's scattered message wording (`_transient_error_message`, the HTTP/malformed branches) is ported **verbatim, branch-for-branch** — timeout text incl. the `/doctor` tail, `Network error: {e}`, `Model returned malformed output: {e}`, `Provider error (HTTP {code})`. No string-matching against exception text survives on the owned path.
- **Single classification site (CD-M-2, `co_cli/agent/loop.py`)** — the provider-error catch moved **inside** `_orchestrator_step_loop` around `_drive_model_request` (the sole site provider errors are raised); the redundant `run_turn_owned` turn-boundary provider catches were removed, leaving only interrupt + a generic last-resort. The generic `Provider error — turn ended:` no longer fires for classified errors.
- **In-loop HTTP recovery** — context overflow strip-then-summarizes via `recover_overflow_history` and retries once (latched per turn, self-committing — the loop only assigns the return, CD-m-4); an HTTP 400 tool-call rejection reflects the graph's reformulation nudge to the model within a per-turn budget (2), then terminates. Overflow never falls through to the 400 path. `overflow_recovery_attempted` + `tool_reformat_budget` added to `TurnState`.
- **Length-continuation retry** — `length_retry_settings` (a `ModelResponse`-shaped port of `_length_retry_settings`): same finish-reason + text-presence + ceiling gate, same ×2 boost ladder (4096→8192→16384), same Ollama `cap_output_tokens` lockstep. On fire the truncated partial is dropped (not appended) and the step re-runs with `settings` boosted in place as a loop-local.
- **Fill-unanswered preflight net (`co_cli/agent/preflight.py`)** — `fill_unanswered_tool_calls` inserts a synthetic `ToolReturnPart` stub `ModelRequest` after any response with unanswered tool calls, run every step (no-op intra-turn; load-bearing only on the first step after an interrupt). `_interrupted_result` now **retains** the unanswered response and appends the abort marker — the deliberate drop→fill divergence (milestone OQ-6, hermes placement); `clean_message_history` merges the stub before the abort prompt, yielding a protocol-valid request.
- **Post-turn output-limit diagnostics** — `_emit_output_limit_diagnostics` ports `_check_output_limits`: the truncation status, the `ctx_overflow_check` span event, and the context-limit / auto-compaction-paused nudges, sourced off the final response's `finish_reason` + per-response `input_tokens` (CD-m-3).
- **Owned-path parity tests (`tests/test_flow_owned_recovery.py`, new)** — 13 tests mirroring the graph-path twins (timeout/network/malformed terminal messages, overflow compact-and-retry then unrecoverable, 400 reflect-within-budget, real-Ollama length-retry convergence, interrupt cross-turn protocol validity, context-limit diagnostics). Error injection via pydantic-ai's `FunctionModel`.
...
- COMPARE: `docs/reference/RESEARCH-behavioral-rules-peer-survey.md` — topic × {co, opencode, hermes-agent, codex, openclaw} coverage matrix (every cell source-cited), gap list (G1 output-formatting confirmed strong, 4/4 peers), and consolidation candidates (C1–C7); head-start hypotheses reconciled (dominant cross-rule duplication confirmed; C3 framing corrected to internal-dup).
