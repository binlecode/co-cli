...
### Removed
- **`update_slug` dedup shortcut**: removed `MemoryCandidate.update_slug` field and the direct `overwrite_memory` branch in `_process_candidate`. All dedup is now handled by the save agent via `persist_memory` with proper resource locking — one dedup path instead of two.
