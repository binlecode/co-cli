#!/usr/bin/env python3
"""Eval: run_reasoning_subagent — verify the thinking subagent is called and returns structured reasoning.

Gates:
  tool_call_rate     >= 1.00  (run_reasoning_subagent called on every case)
...
CASES: list[ThinkCase] = [
    ThinkCase(
        id="rca-ci-timeout",
        prompt=(
            "Use run_reasoning_subagent to reason through this: "
...
async def run_case(
    ...
    think_calls = [(name, args) for name, args in calls if name == "run_reasoning_subagent"]

    # Extract ThinkingResult fields from the ToolReturnPart in message history.
    # ToolReturnPart.content is the dict returned by run_reasoning_subagent.
    plan = ""
    steps: list[str] = []
    conclusion = ""
    for msg in result.messages:
        ...
        for part in msg.parts:
            if not isinstance(part, ToolReturnPart):
                continue
            if part.tool_name != "run_reasoning_subagent":
                continue
    ...
    return {
        ...
        "run_reasoning_subagent_called": len(think_calls) > 0,
...
def compute_metrics(results: list[dict]) -> dict[str, float]:
    ...
    return {
        "tool_call_rate": sum(1 for r in results if r["run_reasoning_subagent_called"]) / n,
...
def print_report(results: list[dict], metrics: dict, failures: list[str]) -> None:
    ...
    for r in results:
        status = "PASS" if (
            r["run_reasoning_subagent_called"]
            and r["plan_nonempty"]
...
async def main() -> int:
    print("=" * 60)
    print("  Eval: Thinking Subagent (run_reasoning_subagent)")
    print("=" * 60)
    ...
    if not registry.is_configured(ROLE_REASONING):
        print(f"\nSKIP: ROLE_REASONING not configured — run_reasoning_subagent cannot run.")
        return 0
    ...
    for i, case in enumerate(CASES, 1):
        ...
        try:
            ...
            results.append(r)
            status = "PASS" if (
                r["run_reasoning_subagent_called"] and r["plan_nonempty"]
            ...
            print("  → TIMEOUT (180s)")
            results.append({
                "id": case.id, "tool_names": [], "run_reasoning_subagent_called": False,
