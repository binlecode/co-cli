...
Create `docs/REPORT-clean-tests-<YYYYMMDD-HHMMSS>.md` at the start of Phase 0 (timestamp fixed at invocation). Track: current phase, files processed (`[x]` clean / `[!]` violation with brief note), findings (file, line, rule, severity, status), and fixes applied. Update after each file read and after each fix — do not batch.
...
2. Match the assertion's intent to one of that workflow's **Primary failure modes** bullets. Encode as `"<workflow_id>::<short_mode_phrase>"` — copying the phrase verbatim from the registry bullet so two runs converge on the same string.
3. If the assertion defends a failure mode the registry does **not** list, record it as `"novel::<short_phrase>"`. Each novel entry is a signal that the registry is incomplete and should be updated — surface novel modes in the Phase 8 report alongside Registry Gaps.
...
`Subsumed unit test` and `Cross-file overlap` are Blocking findings but cannot be detected by single-file grep. They require correlation across the Phase 2 catalog.
...
3. **Required-test-depth check.** For each Covered workflow, compare the covering test's `assertion_shape` and `uses_real_llm` against the workflow's **Required test depth** field using a **closed set of depth-signal patterns**. The agent does not decide which tokens are load-bearing — the patterns are fixed.
   ...
   A Depth-mismatch is a Minor finding with the ESCALATE tag: the test exists and touches the entry, but doesn't probe what the registry says it must.
...
5. Stop when `audited_count >= ceil(workflows × tests / 3000) × workflows / total_workflows` (rough heuristic: aim for ~1/N coverage per run where N is the cross-product overrun factor; clamped to audit at least 25% of workflows per run).
...
- **Scope drift** — tests that don't map to any registered workflow (treat as trim candidates; new-workflow proposals were already surfaced by Phase 0's registry completeness check)
- **Trim candidates** — tests with Phase 3 findings of type Subsumed, Cross-file overlap, or Happy-path-only
