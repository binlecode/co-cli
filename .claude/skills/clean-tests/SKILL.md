...
| # | Rule | Action |
|---|------|--------|
| 1 | **Mock/patch** — `monkeypatch.setattr`/`.setitem`/`.delitem`, `unittest.mock`, `MagicMock`, hand-rolled fakes replacing production behavior. Input literals and `monkeypatch.setenv`/`delenv` isolation (e.g. `CO_HOME`/`HOME`) are fine; fake return values are not. `monkeypatch.setattr(..., fake)` is the real violation. | Reroute through a real production path, or delete |
