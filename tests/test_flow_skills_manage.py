"""Behavioural tests for the skill write tools and skill_view (hermes-parity write + read surface)."""
...
async def test_create_writes_file(tmp_path: Path) -> None:
    ...
    ctx = _make_ctx(deps)
    result = await skill_create(ctx, name="my-skill", content=_VALID_CONTENT)
...
async def test_edit_bundled_only_skill_fails(tmp_path: Path) -> None:
    ...
    result = await skill_edit(ctx, name="doctor", content=_VALID_CONTENT)
    assert _is_error(result)
...
async def test_patch_multiple_matches_replace_all_false_errors(tmp_path: Path) -> None:
    ...
    deps = _make_deps(tmp_path)
    ctx = _make_ctx(deps)
    await skill_create(ctx, name="multi", content=content)
    result = await skill_patch(
        ctx, name="multi", old_string="foo", new_string="bar", replace_all=False
    )
    assert _is_error(result)
...
async def test_patch_zero_matches_errors(tmp_path: Path) -> None:
    ...
    await skill_create(ctx, name="my-skill", content=_VALID_CONTENT)
    result = await skill_patch(
        ctx, name="my-skill", old_string="NONEXISTENT_STRING_XYZ", new_string="x"
    )
    assert _is_error(result)
...
async def test_delete_unknown_name_errors(tmp_path: Path) -> None:
    ...
    deps = _make_deps(tmp_path)
    ctx = _make_ctx(deps)
    result = await skill_delete(ctx, name="nonexistent-skill-xyz")
    assert _is_error(result)
...
async def test_create_no_size_warning_below_30(tmp_path: Path) -> None:
    """skill_create does not include size_warning when skill count < 30.
