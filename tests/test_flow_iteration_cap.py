"""Tests for iteration cap and tool-call-cap hard-stop in run_turn().

Production paths:
  co_cli/config/llm.py            — LlmSettings.max_iterations_per_turn
...
from co_cli.config.llm import DEFAULT_MAX_ITERATIONS_PER_TURN, LlmSettings
...
def test_max_iterations_default_is_90() -> None:
    """LlmSettings() with no overrides yields max_iterations_per_turn == 90."""
    s = LlmSettings(provider="ollama")
    assert s.max_iterations_per_turn == DEFAULT_MAX_ITERATIONS_PER_TURN
    assert s.max_iterations_per_turn == 90


def test_max_iterations_env_override(tmp_path: Path) -> None:
    """CO_LLM_MAX_ITERATIONS_PER_TURN=7 overrides the default to 7."""
    result = load_config(
        _user_config_path=tmp_path / "settings.json",
        _env={"CO_LLM_MAX_ITERATIONS_PER_TURN": "7"},
    )
    assert result.llm.max_iterations_per_turn == 7


def test_max_iterations_env_zero_disables_cap(tmp_path: Path) -> None:
    """CO_LLM_MAX_ITERATIONS_PER_TURN=0 sets cap to 0 (disabled)."""
    result = load_config(
        _user_config_path=tmp_path / "settings.json",
        _env={"CO_LLM_MAX_ITERATIONS_PER_TURN": "0"},
    )
    assert result.llm.max_iterations_per_turn == 0
...
def _make_capped_deps(max_iterations: int) -> CoDeps:
    """Return CoDeps with max_iterations_per_turn pinned."""
    config = SETTINGS_NO_MCP.model_copy(
        update={
            "llm": SETTINGS_NO_MCP.llm.model_copy(
                update={"max_iterations_per_turn": max_iterations}
            )
        }
    )
    return CoDeps(
...
# ---------------------------------------------------------------------------
# Integration (b): iteration cap via run_turn
#
# Strategy: build a stub agent with one approval-required tool. The model
# returns that tool call twice (iterations 1 and 2), triggering the approval
# loop to run two resume segments. On the third model call, it returns text.
# With max_iterations_per_turn=3, the cap fires after the approval loop.
# ---------------------------------------------------------------------------


def _make_iteration_cap_agent() -> Agent:
    """Agent that accumulates 3 LLM iterations via approval-loop resume segments.
    ...
    Total = 3 ModelResponse messages.  With max_iterations_per_turn=3, the cap
...
@pytest.mark.asyncio
async def test_iteration_cap_fires_after_approval_loop() -> None:
    """run_turn with max_iterations_per_turn=3 must stop after 3 LLM calls.

    The stub model returns approval-required tool calls twice, driving two
    approval-resume segments. After iteration 3 (resume segment 2's final model
    call), the iteration cap fires before the turn can return successfully.
    """
    deps = _make_capped_deps(max_iterations=3)
    agent = _make_iteration_cap_agent()
    ...
    assert any("Iteration cap" in s for s in frontend.statuses), (
        f"status must mention 'Iteration cap'; got statuses: {frontend.statuses}"
    )
    assert turn.llm_iterations >= 3, (
        f"must have run at least 3 LLM iterations; got {turn.llm_iterations}"
    )
...
# MAX_TOOL_CALLS_PER_MODEL_TURN + 1 noop calls in the resume segment.
...
def _make_hard_stop_agent() -> Agent:
    ...
    Protocol:
      ...
      model calls 2-4 — each streams MAX_TOOL_CALLS_PER_MODEL_TURN+1 noop calls
    ...
    """
    call_count = {"n": 0}
    _OVER = MAX_TOOL_CALLS_PER_MODEL_TURN + 1
...
async def test_hard_stop_fires_after_consecutive_violations() -> None:
    ...
    Flow:
      ...
      resume segment  — 3 rounds of MAX_TOOL_CALLS_PER_MODEL_TURN+1 noop calls trigger
    ...
    """
    deps = _make_capped_deps(max_iterations=90)
