...
def test_child_deps_refresh_surfaces_disk_skill_when_parent_registry_stale(
    ...
    assert "pass-a-skill" not in child.skill_catalog

    refresh_skills(child)

    assert "pass-a-skill" in child.skill_catalog
    manifest = render_skill_manifest(child.skill_catalog, child.skills_dir, child.user_skills_dir)
...
def test_child_refresh_does_not_mutate_parent_registry(tmp_path: Path) -> None:
    ...
    Verifies set_skill_catalog rebinds only the receiving deps — the parent
    ...
    deps = _make_deps(tmp_path)
    parent_initial_index = deps.skill_catalog
    ...
    assert deps.skill_catalog is parent_initial_index
    assert deps.skill_catalog == {}
    # Child got a fresh dict.
    assert child.skill_catalog is not parent_initial_index
    assert "pass-a-skill" in child.skill_catalog
