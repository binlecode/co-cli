"""Behavioral tests for the skill curator — state machine, triggers, and CLI."""
...
def _make_state_deps(tmp_path: Path, config=SETTINGS) -> CoDeps:
    """CoDeps with full tool registry for state machine tests."""
...
def _make_deps(tmp_path: Path) -> CoDeps:
    """CoDeps with curator_enabled=True for trigger and CLI tests."""
...
def _make_ctx(deps: CoDeps, message_history=None):
...
def test_archive_and_restore_roundtrip_preserves_content(tmp_path: Path) -> None:
    ...
    deps = _make_state_deps(tmp_path)
...
# ---------------------------------------------------------------------------
# maybe_run_curator — trigger conditions
# ---------------------------------------------------------------------------
