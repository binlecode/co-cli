...
the delegated agent's file_write surfaces as an approval prompt on the parent's scripted frontend.
  ...
  - approve → the side effect happens, AND context isolation holds (the parent history carries
              the delegate summary ToolReturnPart but none of the delegated agent's file_write result);
  - headless via the runtime.frontend-is-None path → the delegated agent's collector still runs and
    auto-denies, no side effect (G1-1: a write-capable delegated agent with no frontend never acts
    unprompted — uses a file_write subtask, gated by the collector alone, not shell whose
    in-body raise would mask a regression). Exercised at the delegate_to_agent boundary,
...
def _agent_write_task(target: Path) -> str:
    return (
...
async def test_delegated_write_denied_blocks_side_effect(tmp_path: Path) -> None:
    """A scripted denial blocks the delegated agent's write on disk; the delegate result is still a
    ...
    assert frontend.approval_calls, "the delegated agent's write surfaced on the parent frontend"
    assert not target.exists(), "the denied write did not create the file"
    assert turn.outcome == "continue", "the turn continues after the delegated agent's denial"
...
async def test_delegated_write_approved_executes_and_isolates(tmp_path: Path) -> None:
    """A scripted approval lets the delegated agent's write happen on disk, and the delegated agent's intermediate
    ...
    assert "file_write" not in return_names, (
        "the delegated agent's file_write result stayed isolated"
    )
...
async def test_delegated_write_headless_parent_auto_denies(tmp_path: Path) -> None:
    """G1-1: with the parent's runtime.frontend None, the delegated agent's collector still runs and
    auto-denies — a write-capable delegated agent never acts unprompted.

    delegate_to_agent reads parent_deps.runtime.frontend (here unset → None) and passes
    propagate_approvals=True with frontend=None, so the delegated agent's collector auto-denies the
    file_write. The file's absence is the side-effect proof; delegated-agent token spend proves the
    delegated agent actually ran and attempted the gated work (not a vacuous no-op or depth refusal).
    """
    await ensure_ollama_warm(TEST_LLM.model, TEST_LLM.host)
    target = tmp_path / "delegated.txt"
    deps = _make_deps(tmp_path)
    ...
    async with asyncio.timeout(LLM_TOOL_CONTEXT_TIMEOUT_SECS * 2):
        summary = await delegate_to_agent(deps, _agent_write_task(target))

    assert not target.exists(), "headless delegated agent auto-denied its write — no side effect"
    assert deps.usage_accumulator.output_tokens > 0, (
        "the delegated agent actually ran (not a no-op)"
    )
