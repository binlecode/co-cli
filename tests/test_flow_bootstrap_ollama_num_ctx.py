...
The Modelfile's ``num_ctx`` must meet or exceed the configured ``max_context_tokens``;
...
def test_ollama_num_ctx_floor_raises_when_undercut():
    """Floor check raises ValueError naming both values when num_ctx < max_context_tokens."""
...
def test_ollama_num_ctx_floor_passes_at_and_above_max_context_tokens():
    """Floor check does not raise when num_ctx equals or exceeds max_context_tokens."""
