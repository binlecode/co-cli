...
Also verifies the memory_manage tool actually accepts and persists source_type —
...
def test_memory_review_prompt_sets_source_type_session_review() -> None:
    ...
    assert "source_type='session_review'" in text, (
        ...
        "source_type='session_review' on every memory_manage call"
    )


@pytest.mark.asyncio
async def test_memory_manage_persists_source_type_session_review(tmp_path: Path) -> None:
    ...
    assert result.metadata is None or not result.metadata.get("error"), (
        f"memory_manage rejected source_type parameter: {result}"
    )
