...
async def test_input_too_large_degrades_to_static_marker_sparing_breaker(monkeypatch):
    ...
    before = deps.runtime.compaction_skip_count

    result, summary_text, input_too_large = await compact_messages(deps, _MESSAGES, _BOUNDS)
    ...
    assert deps.runtime.compaction_skip_count == before, "breaker budget was eroded"
...
async def test_summarizer_error_still_increments_skip_count(monkeypatch):
    ...
    assert _marker_content(result).startswith(STATIC_MARKER_PREFIX)
    assert deps.runtime.compaction_skip_count == before + 1, "skip count did not increment"
