...
- Content-fetch reads (file_read, web_fetch) → `read on demand` / `fetch on demand`
- Populated searches (file_search, web_search) → `re-query on demand`
...
def test_file_search_content_populated_has_re_query_suffix() -> None:
    marker = semantic_marker("file_search", {"content": "auth", "path": "."}, "hit1\nhit2\n")
    assert marker.endswith(" — re-query on demand")


def test_file_search_content_no_matches_has_no_suffix() -> None:
    marker = semantic_marker("file_search", {"content": "auth", "path": "."}, "(no matches)")
    assert "on demand" not in marker
    assert marker.endswith("no matches")


def test_file_search_listing_populated_has_re_query_suffix() -> None:
    marker = semantic_marker("file_search", {"path": "**/*.py"}, "a.py\nb.py\n")
    assert marker.endswith(" — re-query on demand")


def test_file_search_listing_no_files_has_no_suffix() -> None:
    marker = semantic_marker("file_search", {"path": "**/*.py"}, "(empty)")
    assert "on demand" not in marker
