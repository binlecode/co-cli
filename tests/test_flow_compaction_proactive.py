...
@pytest.mark.asyncio
async def test_post_compaction_failure_leaves_runtime_clean(monkeypatch) -> None:
    """If post-compaction work raises, runtime must NOT show compaction-applied.

    compact_under_budget computes savings (calling estimate_message_tokens on
    the result) BEFORE committing runtime. Patch estimate_message_tokens to
    raise on that call — compact_under_budget must propagate the exception
    BEFORE writing any runtime field. The proactive fail-open path then
    returns original messages with runtime untouched.

    Failure mode regression: a partial commit would leave compaction_applied_this_turn
    = True after returning original messages, causing the next turn's processor
    to fast-path on a still-oversized history.
    """
    deps = CoDeps(
        shell=ShellBackend(),
        model=_TIGHT_MODEL,
    ...
    deps.runtime.previous_compaction_summary = "ORIGINAL_SUMMARY"
    ctx = RunContext(deps=deps, model=_TIGHT_MODEL.model, usage=RunUsage())
    messages = _above_threshold_messages()

    import co_cli.context.compaction as compaction_mod

    real_estimate = compaction_mod.estimate_message_tokens
    call_count = {"n": 0}

    def flaky_estimate(msgs):
        call_count["n"] += 1
        # 1st call: proactive_window_processor's tokens_before_local.
        # 2nd call: compact_under_budget's tokens_after for savings — make it
        # raise to simulate any pre-commit post-build failure.
        if call_count["n"] >= 2:
            raise RuntimeError("synthetic post-compaction failure")
        return real_estimate(msgs)

    monkeypatch.setattr(compaction_mod, "estimate_message_tokens", flaky_estimate)

    await ensure_ollama_warm(TEST_LLM.model)
    async with asyncio.timeout(LLM_COMPACTION_SUMMARY_TIMEOUT_SECS):
        result = await proactive_window_processor(ctx, messages)

    assert result is messages, "Fail-open path must return the original messages reference"
    assert deps.runtime.compaction_applied_this_turn is False, (
        "Runtime must not show compaction applied when post-compaction work failed"
    )
    assert deps.runtime.previous_compaction_summary == "ORIGINAL_SUMMARY", (
        "previous_compaction_summary must be unchanged on failure"
    )
