...
async def test_read_file_missing_with_similar_name_suggests(tmp_path):
    ...
    result = await read_file(_make_ctx(tmp_path), path="conifg.yaml")
...
async def test_read_file_missing_no_similar_names(tmp_path):
    ...
    result = await read_file(_make_ctx(tmp_path), path="zzz_totally_different.txt")
...
async def test_read_file_missing_parent_dir_no_crash(tmp_path):
    ...
    result = await read_file(_make_ctx(tmp_path), path="nonexistent_dir/file.txt")
...
async def test_read_file_partial_read_emits_continuation_hint(tmp_path):
    ...
    result = await read_file(_make_ctx(tmp_path), path="long.txt", start_line=1, end_line=10)
...
async def test_read_file_partial_read_at_eof_no_hint(tmp_path):
    ...
    result = await read_file(_make_ctx(tmp_path), path="short.txt", start_line=3, end_line=5)
...
async def test_read_file_full_read_no_hint(tmp_path):
    ...
    result = await read_file(_make_ctx(tmp_path), path="full.txt")
...
async def test_patch_ambiguous_exact_match_guidance(tmp_path):
    ...
    await read_file(ctx, path="dup.txt")

    result = await patch(ctx, path="dup.txt", old_string="foo", new_string="baz")
...
async def test_patch_ambiguous_fuzzy_match_guidance(tmp_path):
    ...
    ctx = _make_ctx(tmp_path)
    await read_file(ctx, path="dup.py")
...
async def test_patch_no_show_diff_default_omits_diff_block(tmp_path):
    ...
    result = await patch(ctx, path="data.txt", old_string="hello", new_string="goodbye")
...
async def test_patch_show_diff_same_string_produces_no_diff(tmp_path):
    ...
    ctx = _make_ctx(tmp_path)
    await read_file(ctx, path="same.txt")
...
async def test_patch_lint_warns_on_invalid_python(tmp_path):
    ...
    ctx = _make_ctx(tmp_path)
    await read_file(ctx, path="bad.py")

    # Introduce an unused import violation (E401 / F401 — ruff will warn)
    result = await patch(ctx, path="bad.py", old_string="x = 1", new_string="x = 1\nimport sys")
...
async def test_patch_lint_clean_python_no_warnings(tmp_path):
    ...
    ctx = _make_ctx(tmp_path)
    await read_file(ctx, path="clean.py")

    result = await patch(ctx, path="clean.py", old_string="x = 1", new_string="x = 2")
...
async def test_patch_lint_skipped_for_non_python(tmp_path):
    ...
    result = await patch(ctx, path="data.txt", old_string="import os", new_string="import sys")
