...
def test_thinking_exhaustion_gate_blocks_thinking_only_parts() -> None:
    """Gate must reject a response whose parts contain only ThinkingPart entries."""
    parts = [ThinkingPart(content="some reasoning")]
    assert not any(isinstance(p, (TextPart, ToolCallPart)) for p in parts)


def test_thinking_exhaustion_gate_passes_on_text_part() -> None:
    """Gate must pass when the response has at least one TextPart."""
    parts = [ThinkingPart(content="some reasoning"), TextPart(content="answer")]
    assert any(isinstance(p, (TextPart, ToolCallPart)) for p in parts)


def test_thinking_exhaustion_gate_passes_on_tool_call_part() -> None:
    """Gate must pass when the response has a ToolCallPart."""
    parts = [ToolCallPart(tool_name="shell", args="{}")]
    assert any(isinstance(p, (TextPart, ToolCallPart)) for p in parts)


def test_thinking_exhaustion_gate_blocks_empty_parts() -> None:
    """Gate must reject an empty parts list."""
    parts: list = []
    assert not any(isinstance(p, (TextPart, ToolCallPart)) for p in parts)
