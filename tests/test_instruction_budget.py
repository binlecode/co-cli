...
2. ``build_profile_overlay(WEAK_LOCAL)`` — the worst-case profile overlay (the
   weak-local profile ships the extra reflex prose); empty today.
3. ``build_toolset_guidance(tool_catalog)`` — toolset-gated guidance
   (``CAPABILITIES_GUIDANCE``; ``MEMORY_GUIDANCE`` was deleted in floor-audit TASK-1).
4. ``load_soul_critique(personality)`` — the always-on ``## Review lens`` critique.
...
from co_cli.context.assembly import build_base_instructions, build_profile_overlay
