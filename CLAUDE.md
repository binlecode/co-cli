...
- **Config precedence**: env vars > `.co-cli/settings.json` (project) > `~/.co-cli/settings.json` (user) > built-in defaults.
- **User-global paths**: all user-global files live under `~/.co-cli/` (overridable via `CO_CLI_HOME` env var). Project-local files live under `.co-cli/`.
...
- **Google credentials**: never configure or inject in tests. They resolve automatically through `google_credentials_path` in settings, `~/.co-cli/google_token.json`, or ADC at `~/.config/gcloud/application_default_credentials.json`.
