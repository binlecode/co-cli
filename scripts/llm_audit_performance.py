#!/usr/bin/env python3
"""Audit LLM runtime performance from the OTel trace DB.

Queries production co-cli chat spans (excludes pytest and eval services) and writes
docs/REPORT-llm-audit-performance-YYYYMMDD-HHMMSS.md.

Usage:
    uv run python scripts/llm_audit_performance.py
    uv run python scripts/llm_audit_performance.py --since 2026-04-01
    uv run python scripts/llm_audit_performance.py --since 2026-04-01 --until 2026-04-30
    uv run python scripts/llm_audit_performance.py --db ~/.co-cli/co-cli-logs.db --out docs/
"""
...
def _perf_flow_latency_section(flow_spans: list[FlowChatSpan]) -> str:
    """Build §8 Per-Flow Latency from test-correlated log spans."""
    if not flow_spans:
        return "## 8. Per-Flow Latency\n\n_No log-correlated spans found._\n"

    by_flow: dict[str, list[float]] = defaultdict(list)
    for span in flow_spans:
        by_flow[span.flow].append(span.duration_ms)
    ...
    return f"## 8. Per-Flow Latency\n\n{table}\n"


def _perf_flow_cost_section(flow_spans: list[FlowChatSpan]) -> str:
    """Build §9 Per-Flow Cost from test-correlated log spans."""
    if not flow_spans:
        return "## 9. Per-Flow Cost\n\n_No log-correlated spans found._\n"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="PATH",
        help="pytest log file; appends §8 Per-Flow Latency and §9 Per-Flow Cost",
    )
    ...
    if log_arg:
        ...
        if not log_path.exists():
            ...
            if log_spans:
                ...
                report += "\n" + _perf_flow_latency_section(flow_spans)
                report += "\n" + _perf_flow_cost_section(flow_spans)
            else:
